...
int main() {
    ...
    const rbfsafe::ContinuousRobotSceneOccupancyBundleLoadOptions robot_scene_load_options;
    ...
    const rbfsafe::RotatingOccupancyPublicationHistoryLoadOptions rotating_occupancy_history_load_options;
    const rbfsafe::RotatingOccupancyPublicationHistoryAudit rotating_occupancy_history_audit;
    const rbfsafe::CoordinatedReservationAgreementLoadOptions coordinated_reservation_load_options;
    const rbfsafe::CoordinatedReservationAgreement coordinated_reservation;
    ...
    return RBFSAFE_VERSION_MAJOR == 4 && RBFSAFE_VERSION_MINOR == 7 && RBFSAFE_VERSION_PATCH == 0 &&
                   ...
                   robot_scene_load_options.maximum_obstacle_occupancies > 0 && deployment_frame.valid() &&
                   ...
                   rotating_occupancy_history_load_options.maximum_publications > 0 &&
                   rotating_occupancy_history_audit.evidence() == rbfsafe::EvidenceLevel::Unknown &&
                   !rotating_occupancy_history_audit.authorizes_execution() &&
                   coordinated_reservation_load_options.maximum_participants > 0 &&
                   coordinated_reservation.evidence() == rbfsafe::EvidenceLevel::Unknown &&
                   !coordinated_reservation.authorizes_execution() &&
                   ...
                   rbfsafe::transparency_leaf_kind_name(rbfsafe::TransparencyLeafKind::RuntimeObservation) ==
                       "runtime_observation" &&
                   rbfsafe::transparency_gossip_status_name(rbfsafe::TransparencyGossipStatus::Consistent) ==
                       "consistent" &&

...
targets `RBFSafe::core`, `RBFSafe::envelope`, `RBFSafe::geometry`,
...
The module-level convenience targets `RBFSafe::applications` and
`RBFSafe::assurance` match `modules/applications.h` and
`modules/assurance.h`. They collect the corresponding fine-grained link
targets without changing their dependency direction.
...
Run `rbfsafe_public_identity_quickstart <new-root-bundle-file>
...
[MoveIt 2 guide](../applications/planning/moveit2.md) for its fail-closed behavior and parameters.

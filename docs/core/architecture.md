...
```text
RBFSafe::core
       |-> RBFSafe::envelope -> RBFSafe::geometry --+
...
Application targets -> RBFSafe::applications
Assurance targets -> RBFSafe::assurance
...
Owns the shared public result and standard-library value types plus the
private deterministic JSON, SHA-256, and binary-persistence infrastructure.
It has no dependency on robot models, workspace envelopes, LECT, or Atlas.

### Workspace envelope

Owns the standalone AABB, OBB, k-DOP, and support-hull workspace values,
support queries, conservative overlap predicates, lower distance bounds, and
their internal JSON codec. It depends only on Core and has no robot-model or
Atlas dependency.

### Geometry

Owns modified-DH forward kinematics, the analytic workspace-frame geometric
Jacobian, robot and scene canonical identity, affine-arithmetic endpoint
propagation, conservative link-envelope construction, certificates, and the
default `RegionValidator`. It depends on Core and Workspace envelope.
...
array position. `LectTree` is mutable; `LectSnapshot` is read-only. It depends
only on Core and is independent of robot geometry.

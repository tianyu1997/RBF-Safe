...
RBF-Safe v0.6 supports versioned static workspace AABB snapshots. Dynamic
updates compare stable obstacle IDs between two snapshots, but do not infer
continuous swept motion. Meshes, poses, self-collision models, and uncertainty
models must be conservatively converted outside the library and documented by
the consumer.
...
validate its old/new versions, digests, change kinds, bounds, order, and hash.

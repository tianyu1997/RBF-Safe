...
在 `examples/core/quickstart.cpp` 中辨认：
...
精读 `include/rbfsafe/modules/core.h` 和 quickstart 中的错误分支。理解 `Result<T>`、`Result<void>`、`StatusCode`、`error()` 和 `value()`，以及为什么读取成功值前必须检查结果。

### 第 5～7 天：类、枚举和现代 C++ 接口

精读 `include/rbfsafe/modules/core.h` 与 `include/rbfsafe/modules/geometry.h`。学习 `struct`、`class`、访问控制、构造函数、`enum class`、静态工厂函数、默认参数、封装和对象不变量。
...
2. `include/rbfsafe/modules/core.h`
3. `include/rbfsafe/modules/geometry.h`
4. `tests/test_geometry.cpp`
5. `src/geometry/model.cpp`
6. `include/rbfsafe/modules/geometry.h`
7. `src/geometry/geometry.cpp`
...
1. `include/rbfsafe/modules/lect.h`
2. `tests/test_lect.cpp`
3. `src/lect/lect.cpp`
4. `include/rbfsafe/modules/geometry.h`
5. `src/geometry/certificate.cpp`
...
1. `include/rbfsafe/modules/atlas.h`
2. `tests/test_atlas.cpp`
3. `src/atlas/atlas.cpp` 中的 `AtlasBuilder::build`
4. `SafeAtlas::contains`
5. `SafeAtlas::connected`
6. `include/rbfsafe/modules/atlas.h`
7. `src/atlas/trajectory.cpp`

...
or storage schema changes. RBF-Safe 5.0 intentionally replaces the former
fine-grained C++ headers with seven dependency-ordered module headers and one
optional OMPL module header. Public declarations are unchanged, but old
include paths are not source compatible. Installed CMake target names and
storage schemas remain unchanged.

Public module headers, the aggregate header, and names exported from
`rbfsafe.__init__` are tracked by the current `data/api_surface_v5.sha256`
snapshot. Preserved v1 through v4 snapshots record the historical contracts;
`tools/check_api_surface.py` selects the snapshot for the library's current
major version.
...
- existing documented C++ declarations, enum values, defaults, target names,
  ...
  retained declarations and occupancy API, remain source compatible;
- new overloads, fields with safe defaults, targets, and Python names may be
  added in a minor release;
- a deprecated API remains functional for the rest of 5.x and may be removed
  only in 6.0;
...
The following installed target names are stable in 5.x:

- `RBFSafe::core`, `RBFSafe::envelope`, `RBFSafe::geometry`,
  `RBFSafe::lect`, `RBFSafe::atlas`;
...
- `RBFSafe::shield`, `RBFSafe::policy`, `RBFSafe::memory`, `RBFSafe::trust`,
  ...
  `RBFSafe::rbfsafe`;
- module aggregates `RBFSafe::applications` and `RBFSafe::assurance`; and
...
[the API overview](../core/api.md). New error context text is not itself a stable
machine-readable contract.

## Evidence compatibility

Numeric values and ordering of `EvidenceLevel` are stable throughout 5.x.
Consumers must compare enum values rather than parsing display names. In 5.x,
